...
class PDFService:
    ...
    def generate_monthly_tax_report_pdf(
        ...
        if getattr(settings, "PDF_WATERMARK_ENABLED", False):
            ...
            c.setFillColorRGB(0.1, 0.3, 0.6, alpha=0.08) if hasattr(c, 'setFillColorRGB') else None
            c.translate(300, 400)
            c.rotate(30)
            c.drawString(-200, 0, settings.PDF_WATERMARK_TEXT[:30])
            c.restoreState()
        c.setFont("Helvetica-Oblique", 9)
        c.drawString(40, y - 10, "Generated by Suoops tax automation module (refunded invoices excluded)")
        c.showPage()
        c.save()
        pdf_bytes = buf.getvalue()
        key = f"tax-reports/{report.user_id}/{report.year}-{report.month}.pdf"
